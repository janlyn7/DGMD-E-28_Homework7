...
function GuessingGame()
{
	...
	{
    	const [theAnswer, setTheAnswer] = useState( props.answer )
		...
		function restart()
		{
			...
			if (hasWon) {
				...
				setTotalGuesses(parseInt(totalGuesses + guesses.length ))
			}
			setHasWon(false)
		}
		...
		function handleNewGameState(e) {
			...
			if (result !== -1) {
				...
				if ((result === 1) || (numGuesses === parseInt(maxGuesses)-1)) {
					if (result === 1) {
    	...
    	function GameForm()
		{
			return (
				<div id='gameboard' className='w3-display-container w3-center'>
					...
					<h6> Number of Attempts: {guesses.length} </h6>
				</div>
			);
		}
			...
			<div id='mainbody' className="w3-display-container w3-round-xlarge w3-padding-large w3-card">
				...
				<form>
					...
					<input type='number' id='minimumField' value= {minNumber}
						   ...
						   onInput={(e) => setMinNumber(e.target.value)}/>
					<br/><br/>
					...
					<input type='number' id='maximumField' value= {maxNumber}
						   className="w3-large w3-input w3-round-large w3-light-gray w3-center w3-padding w3-border"
						   onInput={(e) => setMaxNumber(e.target.value)}/>
					<br/><br/>
					...
					<input type='number' id='maximumGuesses' value= {maxGuesses}
						   className="w3-large w3-input w3-round-large w3-light-gray w3-center w3-padding w3-border"
						   onInput={(e) => setMaxGuesses(e.target.value)}/>
				</form>
				<br/><br/>
			</div>
		);
